...
- **`TIB-GA4-Query`** (`ga4.py`) — terminal GA4 Data API reports (channels/sources/pages/events/custom), OAuth read-only "as her", token auto-refresh (headless-capable after first consent). For the **Website / Blog / AI-search** report sections (traffic, source/medium, install-button & register clicks, top pages, countries). **WORKING as of 2026-09-21** — set up + verified pulling real data (OAuth reusing the Expert tool's client, GA4 property 307498741 = teeinblue.com web). Presets: channels/sources/pages/mypages/fbads/events/custom, filters --page/--medium/--source, and **multi-property** via config `properties` + `--prop web|appstore|all` (web=307498741, appstore=374057963 — both verified pulling data). `fb_ad_medium`=vn-ad for FB-ad tracking. Auth: OAuth now (token.json), can switch to service account (sa.json) for headless. Local repo committed; GitHub remote not created yet (no gh CLI → she makes the empty private repo, then push). **Does NOT have App Store installs** (that's Shopify Partner). Don't use `gq` for installs.
- **`TIB-FB-Ads`** (`fbads.py`) — Meta Marketing API insights from terminal: spend/reach/impressions/clicks/ctr/cpc + results & cost-per-result, by account/campaign/adset/ad/day (presets summary/campaigns/adsets/ads/results). Stdlib only; token via `Authorization: Bearer` (not in URL); config.json holds ad_account_id + access_token (gitignored). Scaffolded 2026-09-21 (Opus), **syntax-checked, NOT run yet** — pending her: ad account id (act_...) + a Meta token with ads_read (Graph API Explorer quick, or a Business-Manager system-user token for non-expiring/headless). Meta tokens expire (~60d personal) unless BM system user. Local repo committed; GitHub remote pending (she creates empty private `TIB-FB-Ads`).
...
1. **Ads performance (FB/Etsy)** — ~~no tool~~ → `TIB-FB-Ads` now scaffolded (pending her Meta token/ad-account-id). _Corrected 2026-09-21: was a gap; tool exists now, just needs setup._ (Etsy ads: also on Meta if run via FB — same tool once token set.)
